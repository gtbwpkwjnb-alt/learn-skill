...
在 `learn-output/.skill_state.json` 持久化环境状态，跨会话记忆：
...
> - 缺 tesseract → 跳过 OCR 和关键帧，继续转录，输出标注"🟡 受限模式（无OCR/无关键帧）"
> - 缺 scenedetect → 跳过关键帧，继续转录
> - 缺 yt-dlp → 尝试 `pip install yt-dlp`，失败则提示手动下载
> - 缺 ffmpeg → 无法提取，报安装命令后终止
> - 缺 playwright → 仅影响抖音兜底路径，其余平台正常
> - 缺 browser_cookie3 → 无法自动提取浏览器 cookie，尝试无 cookie 方式
...
> **注意**：
> - 原始转录不入 final.md，仅作为 AI 分析阶段的输入。
> - 如果以降级模式运行（缺 tesseract/scenedetect），在 final.md 顶部标注 🟡 Degraded Mode。
> - 如果用了 playwright 兜底（绕过 yt-dlp），在 final.md 标注提取方式。

增强模板（中英双语，含全部 v3.6 新区域）：
